...
class QCheckBoxDemo(QWidget):
    ...
    def initUI(self):
        ...
        self.checkBox1 = QCheckBox('复选框控件1') #创建复选框
        self.checkBox1.setChecked(True) #设置默认值，复选框为选中状态
        self.checkBox1.stateChanged.connect(lambda:self.checkboxState(self.checkBox1)) #关联槽
        layout.addWidget(self.checkBox1) #放入水平布局中
        self.checkBox2 = QCheckBox('复选框控件2')
        self.checkBox2.stateChanged.connect(lambda:self.checkboxState(self.checkBox2))
        layout.addWidget(self.checkBox2)
        self.checkBox3 = QCheckBox('半选中')
        self.checkBox3.stateChanged.connect(lambda:self.checkboxState(self.checkBox3))
        self.checkBox3.setTristate(True) #是否拥有三种状态
        self.checkBox3.setCheckState(Qt.PartiallyChecked) #设置复选框为选中状态
